...
Generated SysML v2: 675 lines, pilot-implementation validation **PASSED** (0 errors, 47 warnings).

## Summary

| Status | Elements | Share | Meaning |
|---|---:|---:|---|
| Clean | 427 | 67% | semantics preserved in the generated SysML v2 |
| Lossy | 56 | 9% | emitted, but some information was dropped or approximated (see note) |
| Needs decision | 97 | 15% | emitted in a provisional form; a systems engineer must choose the final v2 construct |
...
| Property (value) | attribute usage | 47 | 47 | 0 | 0 | 0 | untyped in the source model |
| ReferenceProperty | ref part | 40 | 40 | 0 | 0 | 0 |  |
| Association (composite) | (part usage) | 36 | 36 | 0 | 0 | 0 | expressed by the owning block's part usage |
| Activity | action def, renamed | 35 | 28 | 7 | 0 | 0 | 7 action nodes and 0 flows not migrated (activity internals require manual re-modelling); 5 action nodes and 9 flows not migrated (activity internals require manual re-modelling); 8 action nodes and 10 flows not migrated (activity internals require manual re-modelling) |
| Operation | action (nested), renamed | 32 | 0 | 1 | 31 | 0 | v2 has no operations; emitted as nested action, calls must be re-modelled; sibling name clash; emitted as 'sequencing_2' |
...
| Property | ref, renamed | 24 | 0 | 1 | 23 | 0 | typed by Activity; choose part/attribute/ref; typed by Interface; choose part/attribute/ref; typed by AssociationClass; choose part/attribute/ref |
| PartProperty | part usage | 21 | 21 | 0 | 0 | 0 |  |
| Class (unstereotyped) | part def | 18 | 0 | 0 | 18 | 0 | plain UML class: treated as part def; confirm it is structural |
| Connector | connect | 17 | 17 | 0 | 0 | 0 |  |
| ValueType | attribute def | 16 | 16 | 0 | 0 | 0 |  |
| ItemFlow / InformationFlow | #ItemFlow dependency | 16 | 0 | 0 | 16 | 0 | v2 flows need a common part context (connect a.port to b.port inside an owner); ends live in different definitions, so the flow is kept as a traced dependency; v2 flows need a common part context (connect a.port to b.port inside an owner); ends live in different definitions, so the flow is kept as a traced dependency; conveyed item type unresolved |
| Port | port usage | 13 | 13 | 0 | 0 | 0 |  |
| SharedProperty | ref part | 13 | 0 | 13 | 0 | 0 | shared aggregation semantics approximated by reference |
...
| InformationFlow | renamed | 9 | 0 | 9 | 0 | 0 | sibling name clash; emitted as 'flow for Task_2'; sibling name clash; emitted as 'flow for Task_3'; sibling name clash; emitted as 'flow for Task_4' |
...
| Property (redefinition) | - | 3 | 0 | 3 | 0 | 0 | redefinition of 'controller' dropped: redefined feature has a different v2 kind; redefinition of 'memberResource' dropped: redefined feature has a different v2 kind |
| AssociationClass | - | 3 | 0 | 0 | 0 | 3 | no mapping implemented |
| Property (composite) | part usage | 3 | 3 | 0 | 0 | 0 |  |
| Property (expression multiplicity) | attribute usage | 2 | 0 | 2 | 0 | 0 | multiplicity expression 'self.nDims * self.nDims'..'self.nDims * self.nDims' dropped; use a v2 calc/constraint; multiplicity expression 'self.nDims'..'self.nDims' dropped; use a v2 calc/constraint |
...
| Class | renamed | 1 | 0 | 1 | 0 | 0 | sibling name clash; emitted as 'DecisionSupport_2' |
...
| CONTROL::DecisionSupport::Sequencing::FIFO::sequencing | Operation | action (nested) | v2 has no operations; emitted as nested action, calls must be re-modelled |
...
| PLANT::Resource::ISA95ResourceDef::Equipment::controller | Property (redefinition) | - | redefinition of 'controller' dropped: redefined feature has a different v2 kind |
...
| PLANT::Resource::OZONEResourceDef::AggregateResource::resourcePool | Property (redefinition) | - | redefinition of 'memberResource' dropped: redefined feature has a different v2 kind |
| PLANT::Resource::OZONEResourceDef::AggregateResource::resourcePool | SharedProperty | ref part | shared aggregation semantics approximated by reference |
...
| PLANT::Resource::ActiveResource::resource | Property | renamed | sibling name clash; emitted as 'resource_2' |
...
| PLANT::Process::Control | Activity | renamed | sibling name clash; emitted as 'Control_2' |
...
| PLANT::Facility::Layout::Geometry::SquareMatrixOfReals::elements | Property (expression multiplicity) | attribute usage | multiplicity expression 'self.nDims * self.nDims'..'self.nDims * self.nDims' dropped; use a v2 calc/constraint |
| PLANT::Facility::Layout::Geometry::VectorOfReals::elements | Property (expression multiplicity) | attribute usage | multiplicity expression 'self.nDims'..'self.nDims' dropped; use a v2 calc/constraint |
...
| CONTROL::DecisionSupport::Admission::simpleCapacity::SimpleCapacity | Activity | renamed | sibling name clash; emitted as 'SimpleCapacity_2' |
...
| CONTROL::DecisionSupport::Admission::capacitatedPriority::capacitatedPriority | Activity | renamed | sibling name clash; emitted as 'capacitatedPriority_2' |
...
| CONTROL::DecisionSupport::Sequencing::FIFO::sequencing | Operation | renamed | sibling name clash; emitted as 'sequencing_2' |
| CONTROL::DecisionSupport | Class | renamed | sibling name clash; emitted as 'DecisionSupport_2' |
...
| DELS::controller | Property (redefinition) | - | redefinition of 'controller' dropped: redefined feature has a different v2 kind |
| flow for Task | InformationFlow | renamed | sibling name clash; emitted as 'flow for Task_2' |
| flow for Task | InformationFlow | renamed | sibling name clash; emitted as 'flow for Task_3' |
| flow for Task | InformationFlow | renamed | sibling name clash; emitted as 'flow for Task_4' |
| flow for Task | InformationFlow | renamed | sibling name clash; emitted as 'flow for Task_5' |
| flow for Task | InformationFlow | renamed | sibling name clash; emitted as 'flow for Task_6' |
| flow for Task | InformationFlow | renamed | sibling name clash; emitted as 'flow for Task_7' |
| flow for Task | InformationFlow | renamed | sibling name clash; emitted as 'flow for Task_8' |
| flow for Task | InformationFlow | renamed | sibling name clash; emitted as 'flow for Task_9' |
| flow for Task | InformationFlow | renamed | sibling name clash; emitted as 'flow for Task_10' |
